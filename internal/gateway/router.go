...
func NewRouter(options Options, services Services) *chi.Mux {
	...
	router.Group(func(api chi.Router) {
		...
		api.Get("/api/features/readiness", featureReadinessHandler(options, services))
		...
		api.Get("/api/chats/{chat_id}/suggestions", chatSuggestionsHandler(services))
		api.Post("/api/chats/{chat_id}/feedback", chatFeedbackHandler(services))
		...
		api.Get("/api/sessions/{session_id}/approvals", sessionApprovalsHandler(services))
		api.Get("/api/sessions/{session_id}/context-usage", sessionContextUsageHandler(services))
		...
		api.Get("/api/tools", toolListHandler(options, services))
		api.Get("/api/tools/{tool_id}", toolDetailHandler(options, services))
		api.Get("/api/skills", skillListHandler(options))
		api.Post("/api/skills", skillCreateHandler(options))
		api.Post("/api/skills/import", skillImportHandler(options))
		api.Get("/api/skills/{skill_id}", skillDetailHandler(options))
		api.Patch("/api/skills/{skill_id}", skillPatchHandler(options))
		api.Delete("/api/skills/{skill_id}", skillDeleteHandler(options))
		...
		api.Get("/api/agents/templates", agentTemplatesHandler())
		...
		api.Post("/api/agents/{agent_id}/copy", agentCopyHandler(options, services))
		api.Post("/api/agents/{agent_id}/disable", agentEnableHandler(options, false))
		api.Post("/api/agents/{agent_id}/enable", agentEnableHandler(options, true))
		api.Post("/api/agents/{agent_id}/validate", agentValidateHandler())
		api.Post("/api/agents/{agent_id}/test", agentTestHandler(options, services))
		...
		api.Post("/api/orchestration/preview", orchestrationPreviewHandler(options, services))
		api.Post("/api/orchestration/test", orchestrationTestHandler(options, services))
		api.Get("/api/sessions/{session_id}/timeline", sessionTimelineHandler(services))
		api.Get("/api/sessions/{session_id}/todos", sessionTodosHandler(services))
		api.Post("/api/evals/harness", harnessEvalHandler(options, services))
