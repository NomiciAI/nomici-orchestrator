...
8. Open the Artifacts panel, preview the final report, and download any file-backed artifact.
9. Approve or reject the memory proposal.
10. Refresh the browser and verify the same chat, session, tasks, artifacts, tool calls, and blocked actions are still visible.
...
- If a tool loop repeats a failing call or exhausts its round budget, the session enters a blocked state with a visible retry decision.
