# Long-Horizon Capability Roadmap

## Purpose

This document turns Nomici's long-horizon agent surface into a phased implementation roadmap.

Nomici should support complex tasks that unfold across minutes or hours: planning, role handoff, tool use, workspace execution, context compression, user review, artifacts, and later non-CLI entrypoints. The implementation should keep the control-plane boundary clear:
...
Nomici already has several foundations for long-horizon work:
...
| Bash and code execution | CLI runners exist; mediated shell tool is not complete | Add bash tool contract with risk classification, working directory policy, timeout, captured output, and approval flow | Phase 3 |
| Search and fetch | Provider setup is model-focused | Add research tool providers as Tool Packs with auth metadata, request logging, redaction, rate limits, and provider doctor checks | Phase 4 |
| MCP integration | MCP is planned, not a full mediated registry | Add MCP registry, server lifecycle/status, OAuth/secret references, tool allowlists, and per-tool policy metadata | Phase 4 |
| Progressive skills | Packs can install agents; skill registry is planned | Add skill metadata, triggers, risk, required tools, selected-skill trace events, explicit run-level enablement, and progressive context injection | Phase 4 |
| Tool search / deferred tools | No equivalent surface | Add searchable tool/skill catalog so runtimes receive only relevant tools/briefings | Phase 4 |
| Context engineering | Shared Context exists for handoffs | Add session summaries, context-window budgeting, run-state compression, and artifact-backed intermediate summaries | Phase 5 |
...
| Streaming API | Gateway APIs are local and Nomici-specific | Add stable run event stream for Console and adapters; keep compatibility surfaces as integrations, not the internal source of truth | Phase 8 |
| Frontend workspace | Console is read-only overview | Add operational Console views for sessions, tasks, plan review, tools, skills, artifacts, uploads, and channel status | Phases 1-7 |
| Model configuration | Provider profiles exist | Expand provider catalog with context/cost/tool/vision/thinking capabilities and compatibility checks per adapter | Phase 1 |
| Observability | Trace store exists | Add token usage, budget accounting, external trace links, event export, and clearer redaction policy | Phase 5 |
| Guardrails and error handling | Policy engine foundation exists | Add loop detection, budget stop, repeated tool failure handling, retry policy, and guardrail result traces | Phase 5 |
| Embedded client | CLI is primary client | Add a typed local SDK only after Gateway API stabilizes; keep CLI and Console as first clients in v0.1/v0.2 | Phase 8 |
| Deployment | Install script and local Gateway exist | Add Docker Compose only after local control-plane flows are hardened; preserve loopback/token defaults | Phase 8 |
| Auth and public exposure | Gateway token auth exists | Before server/team mode, add OIDC/RBAC/workspaces, stricter CSRF/origin guidance, and channel-specific auth review | Phase 9 |

## Phased Plan

### Phase 0: v0.1 Bootstrap Closure

Goal: finish the current local-first proof slice before adding long-horizon breadth.
...
Goal: turn role-based work decomposition into Nomici packs without hardcoding role names in Gateway.
...
- Add read-only search/fetch tool packs where providers are available.
...
- Add channel config schema and optional channel adapters.
...
- Add LangGraph adapter and optional compatibility API surfaces for integration.
...
| L | Message Gateway Channel Adapters | 6 | Optional channel adapters for external messaging entrypoints |
...
- Do not turn pack role names into hardcoded Gateway concepts.
